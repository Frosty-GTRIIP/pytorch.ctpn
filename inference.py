...
class DetectImg():
    ...
    def detect(self, img):
        ...
        boxes, text_proposals = textdetector.detect(
            textsegs, scores[:, np.newaxis], img_ori.shape[:2])
...
def show_img(save_path, im_file, boxes, text_proposals):
    ...
    for item in text_proposals:
        img_ori = cv2.rectangle(
            img_ori, (item[0], item[1]), (item[2], item[3]), (0, 255, 255))
    img_ori = cv2.resize(img_ori, None, None, fx=1.0 / rw,
                         fy=1.0 / rh, interpolation=cv2.INTER_LINEAR)
    for i, box in enumerate(boxes):
        cv2.polylines(img_ori, [box[:8].astype(np.int32).reshape((-1, 1, 2))], True, color=(0, 255, 0),
                      thickness=2)
    img_ori = cv2.resize(img_ori, None, None, fx=0.9,
                         fy=0.9, interpolation=cv2.INTER_LINEAR)
...
if __name__ == "__main__":
    ...
    model_file = './model_save/mobilenet_v3_large.pth'
    ...
    for file in files:
        bar.update(1)
        fid = open(os.path.join(txt_save_path, 'res_' +
                   file.split('.')[0] + '.txt'), 'w+', encoding='utf-8')
